...
export class Affiliates {
    ...
    getLink(
        ...
        const apiPath = '/affiliates/links/{linkId}'.replace(
            '{linkId}',
    ...
    deleteLink(paramsOrFirst: { linkId: string } | string): Promise<{}> {
        let params: { linkId: string };
    ...
    updateReward(
        paramsOrFirst:
            | { rewardId: string; status: string; organizationId: string }
            | string,
        ...rest: [string?, string?]
    ): Promise<Models.AffiliateReward> {
        let params: {
        ...
        if (typeof rewardId === 'undefined' || rewardId === '') {
            throw new AppwriteException(
                'Missing required parameter: "rewardId"',
            );
        }
        ...
        const apiPath = '/affiliates/rewards/{rewardId}'.replace(
            '{rewardId}',
